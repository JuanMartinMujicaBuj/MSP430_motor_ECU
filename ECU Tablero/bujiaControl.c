...
void bujiaControlInit(uint8_t pin_in, uint8_t rpm_in, uint16_t phi1_in, uint16_t phi2_in)
{
    if (yaInit)
        return;
    yaInit = 1;
    ...
    pin=pin_in;
    bujiaInit(pin,NULL); //set pin as output
...
void bujiaControlWrite(uint8_t rpm_in, uint16_t phi1_in, uint16_t phi2_in)
{
...
void rti_bujia(void)
{
    ...
    if(!noInterrupt) //si no estoy en el medio de una modificacion de phi1, phi2
    {
        ...
        {
            writeBujia(1); //si phi1<=phi<=phi2, prendo la bujia
        }
        else
        {
            writeBujia(0); //si no, apago la bujia
        }
    }
}

...
void MCP2515_CanVariable_init (can_t *can)
{
  ...
  can->dlc = CAN_DLC; //data length code (no. of bytes to send)                  // Länge der Information (0 bis 8)
...
void MCP2515_write (uint8_t addr, uint8_t data)
{
  ...
  SPI_transmit(data);                                                    // sende Daten für diese Adresse, ...
...
void MCP2515_read_many_registers(uint8_t addr, uint8_t length, uint8_t *data)
{
  ...
  SPI_transmit(addr);                                                    // Die dazugehörige Addresse
...
{ 
  ...
  MCP2515_write(MCP2515_CNF2,0xbf); //CNFx para 40 o 5kBPS con                   // angelötet sein. Die Werte für CNF1, CNF2 und CNF3, kann man leicht aus der Microchi-Software bekommen. ...
  MCP2515_write(MCP2515_CNF3,0x07); //un oscilador de 8MHz                       // Dazu lade Programm: "Microchip Can Bit Timing Calculator" runter und führe es als ADMIN aus.-
